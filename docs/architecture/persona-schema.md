...
| banterWordCaps    | object  | Max words for `short`, `medium`, and `long` banter       |
...
The persona model uses a **free-form `systemPrompt`** rather than structured personality fields (tone, humourLevel, etc.). This gives users full control over the DJ's personality through natural language, while `banterWordCaps` provides the runtime length controls for short, medium, and long banter.
...
```json
{
  ...
  "banterWordCaps": { "short": 25, "medium": 45, "long": 80 },
...
- Banter max word caps for short / medium / long
